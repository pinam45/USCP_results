...
cd build/bin
./solver --greedy --rwls --rwls_time=120 --repetitions=1 --instances="${instance}"
...
# Execute
cd build/bin
./solver --output_prefix="${solveroutputfolder}/${instance}-${run_number}_" --greedy --rwls --rwls_steps=100000000 --repetitions=1 --instances="${instance}"

# Supression du dossier de build
rm -r "${cmakefolder}"
