...
# Generate PGO data
cd build/bin
./solver --memetic --memetic_rwls_time=12 --memetic_generations=6 --memetic_crossover="${crossover_operator}" --memetic_wcrossover="${wcrossover_operator}" --repetitions=1 --instances=RAIL507
...
# Execute
cd build/bin
./solver --output_prefix="${solveroutputfolder}/${crossover_operator}/${wcrossover_operator}/${instance}_" --greedy --memetic --memetic_rwls_steps="${rwls_steps}" --memetic_time="${memetic_time}" --memetic_crossover="${crossover_operator}" --memetic_wcrossover="${wcrossover_operator}" --repetitions=1 --instances="${instance}"
